...
// cfg is nexus's environment configuration, loaded via platform/config.
...
func main() {
	...
	logger.Info("starting autorix nexus (hybrid rebac engine)")
	...
	// failure — nexus previously used pgxpool.New directly, which had no
	// retry at all.
	...
	metrics.RegisterPostgresPool(pool, "nexus")

	// 2. Initialize Infrastructure Layer (Repository)
	repo := postgres.NewRepository(pool)

	// 3. Initialize Domain Layer (CEL + Graph)
	celEvaluator, err := caveat.NewCELEvaluator(caveat.WithCaveatGetter(repo))
	if err != nil {
	...
	healthHandler := health.NewHandler(checker, func() health.Info {
		return health.Info{
			...
			Capabilities: []string{"rebac", "abac", "zanzibar"},
			InstanceID:   instanceID,
			StartedAt:    startedAt,
		}
	})
	...
	grpcServer := grpc.NewServer(
		grpc.UnaryInterceptor(metrics.UnaryServerInterceptor("nexus")),
		grpc.StreamInterceptor(metrics.StreamServerInterceptor("nexus")),
	)
	nexusServer := grpcTransport.NewServer(resolver,
		grpcTransport.WithRepository(repo),
		grpcTransport.WithCaveatEvaluator(celEvaluator),
	)
	...
	// can never disagree.
	grpchealth.Register(grpcServer, checker, "autorix.nexus.v1.NexusService")

	// Enable reflection for tools like grpcurl
	reflection.Register(grpcServer)

	// 5. Initialize Transport Layer (REST admin API)
	restServer := httpTransport.NewServer(resolver, repo, healthHandler,
		httpTransport.WithCaveatEvaluator(celEvaluator),
	)
	restHandler := httpx.Chain(restServer.Routes(),
		httpx.RequestID,
		httpx.Recover(logger),
		metrics.HTTPMiddleware("nexus"),
	...
	// nexus previously ran grpcServer.Serve blocking with no signal handling
	...
	registryClient := registry.NewFromEnv("nexus", registry.Endpoints{
	...
	err = run.Run(runCtx, c.ShutdownTimeout, logger, []run.Named{
		...
		},
		{
			Name: "nexus-grpc",
			Serve: func() error {
				logger.Info("nexus listening", "port", c.Port, "transport", "gRPC")
	...
	logger.Info("autorix nexus stopped")
}
