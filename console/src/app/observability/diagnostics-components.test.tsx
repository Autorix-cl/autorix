...
describe("Diagnostics UI Components", () => {
  ...
  it("renders DiagnosticsManager and executes connectivity probe", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/DNS Lookup/i)).toBeInTheDocument();
      expect(screen.getByText(/TCP Connect/i)).toBeInTheDocument();
  ...
  it("triggers diagnostic bundle export", async () => {
    ...
    await waitFor(() => {
      expect(global.fetch).toHaveBeenCalledWith("/api/diagnostics/bundle", { method: "POST" });
    });
  });
});
