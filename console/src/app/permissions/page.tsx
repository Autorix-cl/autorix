...
import { SchemaEditor } from "@/app/nexus/schema-editor";
import { RelationshipGraph } from "@/app/nexus/relationship-graph";
import { CaveatRegistry } from "@/app/nexus/caveat-registry";
import { useNexusSchema, useSaveNexusSchema } from "@/hooks/use-nexus";
  ...
  return (
    <div className="space-y-6">
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              className="h-8 gap-1.5 text-xs bg-purple-600 hover:bg-purple-700 text-white font-medium shadow-sm shrink-0"
            >
      ...
      <Tabs defaultValue="simulator" className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-border pb-3">
          <TabsList className="bg-muted/40 p-1">
            <TabsTrigger value="simulator" className="gap-2 text-xs font-medium">
              ...
              <span>{t("permissions.simulatorTitle")}</span>
            </TabsTrigger>
            <TabsTrigger value="tuples" className="gap-2 text-xs font-medium">
              <Database className="h-3.5 w-3.5 text-purple-400" />
              <span>Active Relation Tuples</span>
            ...
            <TabsTrigger value="schema" className="gap-2 text-xs font-medium">
              ...
              <span>Zanzibar Schema</span>
            </TabsTrigger>
            <TabsTrigger value="graph" className="gap-2 text-xs font-medium">
              <GitGraph className="h-3.5 w-3.5 text-emerald-400" />
              <span>Relationship Graph</span>
            </TabsTrigger>
            <TabsTrigger value="caveats" className="gap-2 text-xs font-medium">
              <ShieldAlert className="h-3.5 w-3.5 text-amber-400" />
              <span>CEL Caveats</span>
            </TabsTrigger>
          </TabsList>
        </div>

        {/* Tab 1: Evaluation Simulator */}
        ...
        {/* Tab 2: Relation Tuples Table */}
        <TabsContent value="tuples" className="space-y-4 focus-visible:outline-none">
          <CloudSection
        ...
        {/* Tab 3: Schema Editor */}
        <TabsContent value="schema" className="space-y-4 focus-visible:outline-none">
          <CloudSection
            title="Zanzibar Authorization Schema"
            description="Declarative Zanzibar DSL definitions defining namespaces, relations, and usersets"
            icon={FileCode}
            badge="Zanzibar DSL"
            badgeVariant="purple"
          >
            {isLoadingSchema ? (
              <div className="flex h-96 items-center justify-center rounded-lg border bg-card/50">
        ...
        <TabsContent value="graph" className="space-y-4 focus-visible:outline-none">
          <CloudSection
            ...
            description="Visual neighborhood exploration of Zanzibar object-relation-subject trees"
            icon={GitGraph}
            badge="Topology View"
            badgeVariant="purple"
          >
        ...
        {/* Tab 5: CEL Caveats */}
        <TabsContent value="caveats" className="space-y-4 focus-visible:outline-none">
          <CloudSection
            title="CEL Caveats Registry"
            description="Google Common Expression Language (CEL) runtime conditions attached to relation tuples"
            icon={ShieldAlert}
            badge="Google CEL"
            badgeVariant="purple"
          >
